...
// Html2pdf options
option = {
    scale: 3, // Increase the scale for higher resolution (e.g., 2 for 2x)
    useCORS: true, // Enable Cross-Origin Resource Sharing (CORS) for external assets
    dpi: 300, // Set the resolution (dots per inch) for the canvas
  };

// Function to call areaCv and Html2Pdf options 
function generateResume(){
    window.jsPDF = window.jspdf.jsPDF;
    html2canvas(areaCv, option).then(function (canvasImage) {
        // Create a new jsPDF instance
        const pdf = new jsPDF('p', 'mm', [297, 210]);
      
        // Add the canvas image as a new page in the PDF
        pdf.addImage(canvasImage, "JPEG", 0, 0, 210, 297);
      
        // Save the PDF file
        pdf.save("Mohan_Resume.pdf");})
}
